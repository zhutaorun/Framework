...
namespace SDGame.UITools
{
    ...
    public class UIControlData : MonoBehaviour
    {
        ...
        [NonSerialized]
        public List<WeakReference> bindUIRefs;


        /// <summary>
        /// 缓存所有打开过的UI类型的字段数据(Todo 是否需要在特定时机清理以节约内存)
        /// </summary>
        ...
        /// 忽略没有正确绑定UI组件错误提示
        /// </summary>
        private bool _ignoreNotFindBindingControlErrorTips;
        ...
        /// 忽略没有正确绑定UI组件错误提示
        /// </summary>
        /// <param name="ignoreNotFindBindingControlErrorTips">是否忽略UI没有正确绑定UI组件错误提示</param>
        public void SetIngnoreNotFindBindingControlErrorTips(bool ignoreNotFindBindingControlErrorTips)
        {
            _ignoreNotFindBindingControlErrorTips = ignoreNotFindBindingControlErrorTips;
        }
...
#endif   
            UIFieldsInfo fieldsInfo = GetUIFieldsInfo(ui.GetType());
            var controls = fieldsInfo.controls;
            for (int i = 0, imax = controls.Count; i < imax; i++)
                BindCtrl(ui,controls[i]);

            var subUIs = fieldsInfo.subUIs;
            for (int i = 0, imax = subUIs.Count; i < imax; i++)
                BindSubUI(ui, subUIs[i]);
            if (bindUIRefs == null)
                bindUIRefs = new List<WeakReference>();
            bindUIRefs.Add(new WeakReference(ui));
#if DEBUG_LOG
            Profiler.EndSample();
        ...
        public static UIFieldsInfo GetUIFieldsInfo(Type type)
        {
            ...
            FieldInfo[] fis = type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);
