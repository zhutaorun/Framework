...
namespace UITool.Editor
{ 
    public class AutoMakerCustomFont
    {
        //本方法是通过裁切的sprite导出字体文件,裁切使用的是Unity自带的sprite editor,方便操作
        //另外，裁切之后,每个sprite的名字的最后一个字符对应了ascii码的编码,例如，0：我们只要将sprite的名字命名成xxx0.就可以了！
        //由于使用到了sprite加载,所以字体图片请放在Resourecs目录下面。等制作完毕，再把他们放到fonts文件下或者其他文件夹中即可

        [MenuItem("Assets/CreateCustonFontSprite")]
        static void CreateMyFontSprite()
        {
            if (Selection.objects == null) return;
            if(Selection.objects.Length ==0)
            {
                Debug.LogWarning("没有选中sprite文件,需要将Sprite Mode设置成Multiple,切分好，并且以名字的最后一个字符当做ascii码");
                return;
            }
            string resoursePath = "Resourecs";
            UnityEngine.Object o = Selection.objects[0];
            if(o.GetType()!= typeof(Texture2D))
            {
                Debug.LogWarning("选中的并不是图片文夹");
                return;
            }
            ...
            if (selectionPath.Contains(resoursePath))
            {
                ...
                float lineSpace = 0.1f;//字体行间距，下面会根据最高的字体得到行间距，如果是固定高度,可以在这里调整
                loadPath = Path.GetFileNameWithoutExtension(selectionPath);
                Sprite[] sprites = Resources.LoadAll<Sprite>(loadPath);
                if (sprites.Length > 0)
                {
                    //以texture方式获得该资源，可以设置到创建的材质中去
                    ...
                    for (int i = 0; i < sprites.Length; i++)
                    {
                        ...
                        Rect rect = spr.rect;
                        //根据pivot设置字符的偏移，具体需要做什么样的，可以根据自己需要修改公式
                    ...
                    //由于上面fresh之后在编辑器中依然没有刷新，所有暂时想到这个方法,
                    //先把生成的字体导出一个包，然后再重新导入进来，这样就直接刷新了
                    ...
                    //打印是为了是使用者方便填写行高。因为font不支持设置行高
                    Debug.Log("创建字体成功，最大高度：" + lineSpace + ",最佳行高：" + (lineSpace + 2));
                }
                else
                {
